...
class TabDataReprGen:
    ...
    def preprocess_audio(self, data):
        ...
        elif self.preproc_mode == 'cens':
            data = np.abs(librosa.feature.chroma_cens(data, sr=self.sr_curr, hop_length=self.hop_length, bins_per_octave=self.cqt_bins_per_octave))
        else:
    ...
    def get_nth_filename(self, n):
        ...
        filenames = [x for x in filenames if x[-5:] == ".jams"]
